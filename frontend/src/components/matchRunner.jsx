...
// --- SVG Loading Spinner ---
// This is a self-contained, animated SVG that doesn't rely on external CSS.
            ...
            <div className="mt-8 text-center">
                {loading ? (
                    <div className="flex flex-col items-center justify-center" aria-live="polite" aria-busy="true">
                        <LoadingSpinner />
                        <p className="mt-4 text-gray-600">Reconciling... this may take a moment.</p>
                    </div>
                ) : (
                    <button 
                        onClick={onRun}
                        disabled={!filesSelected || loading} // Also disable while loading
                        className="w-full bg-blue-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-blue-700 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 disabled:bg-gray-400 disabled:cursor-not-allowed"
                    >
                        Run Reconciliation
                    </button>
                )}
